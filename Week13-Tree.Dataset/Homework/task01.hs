...
twoDigitsAfrerDecimalPoint :: (RealFrac a) => a -> a
twoDigitsAfrerDecimalPoint = (/100) . fromIntegral . round . (*100)

average :: (RealFrac a) => BTree a -> a
average tree = twoDigitsAfrerDecimalPoint (sumTree tree / (fromIntegral (size tree)))
